...
#       ))
#     out <- fit_model_r(df = sim_list$responses, nclass=2, class2domain = c(0,0), nitr= nitr, class_init_method = "kmodes", domain_proposal_ratio=0.5)
#     sink.reset()
#   }
# )
...
#         c(rep(0.2, 10), rep(0.8, 10))
#       ))
#     out <- fit_model_r(df = sim_list$responses, nclass=1, class2domain = c(0), nitr= nitr, class_init_method = "kmodes", domain_proposal_ratio=0.5)
#     sink.reset()
#   }
# )
...
system.time(
  {
    ...
    set.seed(20)
    out <- dependentLCM_fit(df = sim_list$responses3, nclass=1, class2domain = c(0), nitr= nitr, class_init_method = "kmodes", domain_proposal_ratio=0.8)
